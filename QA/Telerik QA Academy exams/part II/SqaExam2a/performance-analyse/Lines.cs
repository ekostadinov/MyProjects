...
class Lines
{
    ...
    static void Main()
    {
        ...
        if (lineMaxLen > 1)
        {
            Console.WriteLine("Max line length = {0}. Lines count = {1}.", 
				lineMaxLen, linesCount);
            stopwatch.Stop();
    ...
    {
        using (StreamReader reader = new StreamReader(@"C:\Users\User\Documents\GitHub\MyProjects\QA\Telerik QA Academy exams\part II\SqaExam2a\performance-analyse\input.txt"))
		{
			// Read the cuboid size
